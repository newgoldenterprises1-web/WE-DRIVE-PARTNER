...
class PremiumBookingDetailScreen extends StatefulWidget {
  final Booking booking;

  const PremiumBookingDetailScreen({
    super.key,
    required this.booking,
  });

  @override
  State<PremiumBookingDetailScreen> createState() {
    return _PremiumBookingDetailScreenState();
  }
}

class _PremiumBookingDetailScreenState extends State<PremiumBookingDetailScreen> {
  late String status;
  ...
  Future<void> _fetchExistingInspectionData() async {
    try {
      ...
      if (doc.exists && mounted) {
        final data = doc.data();
        if (data != null) {
          print("Existing premium inspection data retrieved successfully.");
        }
      }
    } catch (e) {
      print('Error fetching premium inspection data: $e');
    }
  }
  ...
  Future<String?> _uploadImageToStorageWithRetry(File imageFile, String folderName, {int retries = 3}) async {
    for (int i = 0; i < retries; i++) {
      try {
        ...
        if (i == retries - 1) {
          print('Upload failed after $retries attempts: $e');
          return null;
        }
    ...
    try {
      ...
      try {
        ...
        if (query.docs.isNotEmpty) {
          ...
          await query.docs.first.reference.set(fallbackData, SetOptions(merge: true));
        }
          ...
          : ListView(
              ...
              children: [
                AppCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text('Date & Time: ${b.date} • ${b.time}'),
                      const SizedBox(height: 4),
                      Text('Booking ID: ${b.id}'),
                      const SizedBox(height: 4),
                      Text('Estimated Earnings: ₹${b.earnings ?? "1450"}'),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                ...
                if (currentStatus == 'ARRIVED') ...[
                  AppPrimaryButton(
                    label: areAllPreTripPhotosCaptured ? 'START PREMIUM TRIP' : 'COMPLETE PHOTOS TO START TRIP',
                ...
                if (showNavigate) ...[
                  AppOutlineButton(
                    ...
                    onPressed: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Navigation action ready for Maps integration.'),
                          behavior: SnackBarBehavior.floating,
                        ),
                      );
                    },
