...
class _PremiumBookingsScreenState extends State<PremiumBookingsScreen> {
  ...
  Future<void> _startPremiumPayment() async {
    ...
    if (success && paymentService.isTestMode) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('TEST PAYMENT SUCCESSFUL • No real money charged. Premium is still locked until server verification.'), backgroundColor: Colors.blueGrey));
    } else if (success) {
      await _loadPlan();
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Premium Drive activated successfully.')));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment was not completed or could not be verified.'), backgroundColor: Colors.orange));
    }
  ...
  Booking _bookingFromDoc(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    final d = doc.data();
    final fare = d['fare'] is num ? (d['fare'] as num).toDouble() : d['estimatedFare'] is num ? (d['estimatedFare'] as num).toDouble() : 0.0;
    final createdAt = d['createdAt'];
    return Booking(
      id: doc.id,
      customer: (d['customerName'] ?? 'Passenger').toString(),
      date: createdAt is Timestamp ? _date(createdAt.toDate()) : 'Upcoming',
      time: createdAt is Timestamp ? _time(createdAt.toDate()) : 'Scheduled',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance.collection('bookings').where('status', whereIn: const ['REQUESTED', 'SEARCHING']).where('isPremiumBooking', isEqualTo: true).snapshots(),
        builder: (context, pendingSnapshot) {
          final immediate = (pendingSnapshot.data?.docs ?? []).map(_bookingFromDoc).toList();
          return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            stream: FirebaseFirestore.instance.collection('bookings').where('partnerId', isEqualTo: uid).snapshots(),
            builder: (context, assignedSnapshot) {
              final assigned = (assignedSnapshot.data?.docs ?? []).where((d) => d.data()['isPremiumBooking'] == true).map(_bookingFromDoc).toList();
              ...
              return Column(children: [
                ...
                Container(margin: const EdgeInsets.symmetric(horizontal: 18), padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: AppColors.navy, borderRadius: BorderRadius.circular(20)), child: const Row(children: [CircleAvatar(backgroundColor: AppColors.gold, child: Icon(Icons.workspace_premium_rounded, color: AppColors.navyDark)), SizedBox(width: 14), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('PREMIUM PARTNER', style: TextStyle(color: AppColors.gold, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 1.2)), SizedBox(height: 3), Text('High-value corporate rides • 85% share', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 14))]))]),
  ...
  Widget _upgradeView() {
    return Scaffold(backgroundColor: const Color(0xFFF7F9FC), appBar: AppBar(title: const Text('Premium Drive'), elevation: 0), body: ListView(padding: const EdgeInsets.fromLTRB(18, 24, 18, 28), children: [Container(padding: const EdgeInsets.all(22), decoration: BoxDecoration(color: AppColors.navy, borderRadius: BorderRadius.circular(22)), child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(Icons.workspace_premium_rounded, color: AppColors.gold, size: 42), SizedBox(height: 14), Text('Become a Premium Chauffeur', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)), SizedBox(height: 8), Text('Pay the Premium Drive fee to unlock exclusive corporate bookings. Driver earnings remain at the standard 85% share.', style: TextStyle(color: Colors.white70, fontSize: 13, height: 1.4))])), const SizedBox(height: 16), AppCard(child: Column(children: [_benefit(Icons.business_center_rounded, 'Premium corporate bookings'), _benefit(Icons.bolt_rounded, 'Priority allocation opportunities'), _benefit(Icons.payments_rounded, '85% driver earnings share'), const Divider(height: 28), const Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Premium Drive fee', style: TextStyle(color: AppColors.navy, fontWeight: FontWeight.w800)), Text('₹699', style: TextStyle(color: AppColors.gold, fontSize: 22, fontWeight: FontWeight.w900))])])), const SizedBox(height: 18), SizedBox(width: double.infinity, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: AppColors.navy, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0), onPressed: processing ? null : _startPremiumPayment, child: processing ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text('PAY ₹699 & UPGRADE', style: TextStyle(fontWeight: FontWeight.w900)))), const SizedBox(height: 10), Text(RazorpayPaymentService.testMode ? 'TEST MODE: no real charge and no Premium activation until server verification is connected.' : 'Premium bookings remain locked until the Premium Drive payment is verified and the partner account is marked Premium.', textAlign: TextAlign.center, style: const TextStyle(color: AppColors.muted, fontSize: 11, height: 1.4))]));
  }
  ...
  Widget premiumCard(Booking booking) => AppCard(onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => PremiumBookingDetailScreen(booking: booking))), color: const Color(0xFFFEFBF2), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const Icon(Icons.workspace_premium_rounded, color: AppColors.gold, size: 20), const SizedBox(width: 6), const Text('PREMIUM BOOKING', style: TextStyle(color: AppColors.navy, fontWeight: FontWeight.w900, fontSize: 12)), const Spacer(), const Text('85% SHARE', style: TextStyle(color: AppColors.green, fontSize: 10, fontWeight: FontWeight.w900))]), const SizedBox(height: 14), Row(children: [Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppColors.navy.withOpacity(.08), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.person_rounded, color: AppColors.navy)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(booking.customer, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: AppColors.navy)), const SizedBox(height: 2), Text('${booking.date} • ${booking.time}', style: const TextStyle(color: AppColors.muted, fontSize: 12))])), Text('₹${booking.earnings}', style: const TextStyle(color: AppColors.navy, fontSize: 18, fontWeight: FontWeight.w900))]), const SizedBox(height: 12), const Divider(height: 1), const SizedBox(height: 12), Text('${booking.pickup} → ${booking.destination}', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.navy, fontSize: 13, fontWeight: FontWeight.w600))]));
}
