...
class _PremiumBookingsScreenState extends State<PremiumBookingsScreen> {
  ...
  Future<void> _startPremiumPayment() async {
    ...
    if (success && paymentService.isTestMode) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('TEST PAYMENT SUCCESSFUL • No real money charged. Premium is still locked until server verification.'),
      ...
      await _loadPlan();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Premium Drive activated successfully.')));
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Booking _bookingFromDoc(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    final d = doc.data();
    final rawFare = d['fare'] ?? d['estimatedFare'];
    final fare = rawFare is num ? rawFare.toDouble() : 0.0;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: const [
          Padding(
            ...
            child: Icon(Icons.workspace_premium_rounded, color: AppColors.gold),
          ),
        ],
      ),
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance
            ...
            .where('isPremiumBooking', isEqualTo: true)
            .snapshots(),
        builder: (context, pendingSnapshot) {
          final immediate = (pendingSnapshot.data?.docs ?? <QueryDocumentSnapshot<Map<String, dynamic>>>[])
              .map(_bookingFromDoc)
              .toList();
          ...
          return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, assignedSnapshot) {
              final assigned = (assignedSnapshot.data?.docs ?? <QueryDocumentSnapshot<Map<String, dynamic>>>[])
                  .where((doc) => doc.data()['isPremiumBooking'] == true)
                  .map(_bookingFromDoc)
                  .toList();
              final upcoming = assigned.where((b) => ['ACCEPTED', 'ARRIVING', 'ARRIVED', 'TRIP_STARTED'].contains(b.status)).toList();
                    ...
                    child: const Row(
                      children: [
                        CircleAvatar(
                          ...
                          child: Icon(Icons.workspace_premium_rounded, color: AppColors.navyDark),
                        ),
                        SizedBox(width: 14),
                        Expanded(
                          child: Column(
                  ...
                  Expanded(
                    child: current.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (_, i) => premiumCard(current[i]),
                          ),
                  ),
                ],
              );
        ...
        children: [
            ...
            child: Column(
              children: [
                ...
                const Row(
                  ...
                  children: [
                    Text('Premium Drive fee', style: TextStyle(color: AppColors.navy, fontWeight: FontWeight.w800)),
                    Text('₹699', style: TextStyle(color: AppColors.gold, fontSize: 22, fontWeight: FontWeight.w900)),
                  ],
                ),
              ],
            ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              onPressed: processing ? null : _startPremiumPayment,
              child: processing
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Text(text, style: const TextStyle(color: AppColors.navy, fontWeight: FontWeight.w600, fontSize: 13))),
        ],
      ),
    );
  }
        ...
        child: TextButton(
          ...
          style: TextButton.styleFrom(
            ...
            shape: RoundedRectangleBorder(
              ...
              side: BorderSide(color: selected ? AppColors.navy : Colors.grey.shade300),
            ),
          ),
        ...
        children: [
          Row(
            children: [
              ...
              const Text('PREMIUM BOOKING', style: TextStyle(color: AppColors.navy, fontWeight: FontWeight.w900, fontSize: 12)),
              const Spacer(),
              const Text('85% SHARE', style: TextStyle(color: AppColors.green, fontSize: 10, fontWeight: FontWeight.w900)),
            ],
          ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('${booking.date} • ${booking.time}', style: const TextStyle(color: AppColors.muted, fontSize: 12)),
                  ],
                ),
              ),
