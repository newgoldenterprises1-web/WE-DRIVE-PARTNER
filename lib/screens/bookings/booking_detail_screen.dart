...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
  ...
  Future<void> _fetchCustomerPhone() async {
    try {
      final snapshot = await FirebaseFirestore.instance.collection('bookings').doc(widget.booking.id).get();
      if (!mounted || !snapshot.exists) return;
      final data = snapshot.data() ?? <String, dynamic>{};
      final rawPhone = data['customerPhone'] ?? data['customerMobile'] ?? data['phone'] ?? data['mobile'];
      final normalized = rawPhone?.toString().trim();
      if (normalized == null || normalized.isEmpty) return;
      setState(() => customerPhone = normalized);
    } catch (e) {
  ...
  Future<void> _contactCustomer() async {
    ...
    if (phone.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Customer phone number is unavailable.'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
      return;
    }
    try {
      final launched = await launchUrl(Uri(scheme: 'tel', path: phone), mode: LaunchMode.externalApplication);
      if (!launched && mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not open the phone dialer.'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not start the call: $e'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
    }
  }
  ...
  Future<void> _openMapNavigation(String destinationAddress) async {
    ...
    if (address.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Pickup location is unavailable.'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      if (!launched) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not open Google Maps navigation.'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Navigation error: $e'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
    } finally {
  ...
  Future<void> _capturePhoto(String type) async {
    try {
      ...
      setState(() {
        if (type == 'Front') { preTripFront = File(image.path); preTripFrontUrl = null; }
        if (type == 'Back') { preTripBack = File(image.path); preTripBackUrl = null; }
        if (type == 'Right') { preTripRight = File(image.path); preTripRightUrl = null; }
        if (type == 'Left') { preTripLeft = File(image.path); preTripLeftUrl = null; }
        if (type == 'Selfie') { driverSelfie = File(image.path); driverSelfieUrl = null; }
        if (type == 'PostFront') { postTripFront = File(image.path); postTripFrontUrl = null; }
        if (type == 'PostBack') { postTripBack = File(image.path); postTripBackUrl = null; }
        if (type == 'PostRight') { postTripRight = File(image.path); postTripRightUrl = null; }
        if (type == 'PostLeft') { postTripLeft = File(image.path); postTripLeftUrl = null; }
      });
      if (!mounted) return;
  ...
  bool get areAllPreTripPhotosCaptured =>
      (preTripFront != null || preTripFrontUrl != null) &&
  ...
  bool get areAllPostTripPhotosCaptured =>
      (postTripFront != null || postTripFrontUrl != null) &&
  ...
  Future<String?> _uploadImageToStorageWithRetry(File imageFile, String folderName, {int retries = 3}) async {
    for (int i = 0; i < retries; i++) {
      try {
        ...
        final ref = FirebaseStorage.instance.ref().child('inspections').child(widget.booking.id).child(folderName).child(fileName);
  ...
  Future<void> changeStatus(String nextStatus) async {
    ...
    if (!BookingStatusService.canTransition(status, target)) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Invalid booking transition: $status → $target'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
      return;
    }
    setState(() => isLoading = true);
    try {
      ...
      if (target == 'TRIP_STARTED') {
        if (!areAllPreTripPhotosCaptured) throw StateError('missing_pre_trip_photos');
        if (preTripFrontUrl == null) preTripFrontUrl = await _uploadImageToStorageWithRetry(preTripFront!, 'pre_trip');
        if (preTripBackUrl == null) preTripBackUrl = await _uploadImageToStorageWithRetry(preTripBack!, 'pre_trip');
        if (preTripRightUrl == null) preTripRightUrl = await _uploadImageToStorageWithRetry(preTripRight!, 'pre_trip');
        if (preTripLeftUrl == null) preTripLeftUrl = await _uploadImageToStorageWithRetry(preTripLeft!, 'pre_trip');
        if (driverSelfieUrl == null) driverSelfieUrl = await _uploadImageToStorageWithRetry(driverSelfie!, 'selfies');
        ...
        if (updateData.values.any((value) => value == null)) throw StateError('pre_trip_upload_failed');
      }
      if (target == 'COMPLETED') {
        if (!areAllPostTripPhotosCaptured) throw StateError('missing_post_trip_photos');
        if (postTripFrontUrl == null) postTripFrontUrl = await _uploadImageToStorageWithRetry(postTripFront!, 'post_trip');
        if (postTripBackUrl == null) postTripBackUrl = await _uploadImageToStorageWithRetry(postTripBack!, 'post_trip');
        if (postTripRightUrl == null) postTripRightUrl = await _uploadImageToStorageWithRetry(postTripRight!, 'post_trip');
        if (postTripLeftUrl == null) postTripLeftUrl = await _uploadImageToStorageWithRetry(postTripLeft!, 'post_trip');
        ...
        if (updateData.values.any((value) => value == null)) throw StateError('post_trip_upload_failed');
      }
      final updated = await BookingStatusService.updateStatus(bookingId: widget.booking.id, nextStatus: target);
      if (!updated) throw StateError('status_update_rejected');
      if (updateData.isNotEmpty) {
        await FirebaseFirestore.instance.collection('bookings').doc(widget.booking.id).set(updateData, SetOptions(merge: true));
      }
      if (!mounted) return;
      setState(() { status = target; widget.booking.status = target; });
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Booking status updated to: $target'), behavior: SnackBarBehavior.floating));
    } catch (e) {
      if (!mounted) return;
      var message = 'Error updating booking status.';
      if (e is StateError) {
        switch (e.message) {
          case 'missing_pre_trip_photos': message = 'Please capture all pre-trip photos and driver selfie first.'; break;
          case 'missing_post_trip_photos': message = 'Please capture all post-trip photos first.'; break;
          case 'pre_trip_upload_failed': message = 'Pre-trip inspection upload failed. Please retry.'; break;
          case 'post_trip_upload_failed': message = 'Post-trip inspection upload failed. Please retry.'; break;
          case 'status_update_rejected': message = 'Booking is no longer available for this action.'; break;
        }
      }
  ...
  Widget _buildPhotoRow(String title, File? file, String? remoteUrl, String typeKey) {
    final isCaptured = file != null || (remoteUrl != null && remoteUrl.isNotEmpty);
    ImageProvider? imageProvider;
    if (file != null) imageProvider = FileImage(file);
    if (imageProvider == null && remoteUrl != null && remoteUrl.isNotEmpty) imageProvider = NetworkImage(remoteUrl);
    return Container(
        ...
        children: [
          Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: isCaptured ? Colors.green.withOpacity(0.08) : AppColors.muted.withOpacity(0.2),
                borderRadius: BorderRadius.circular(6),
                image: imageProvider == null ? null : DecorationImage(image: imageProvider, fit: BoxFit.cover),
              ),
              child: imageProvider == null ? const Icon(Icons.camera_alt, size: 20, color: AppColors.muted) : null,
            ),
          ...
          ElevatedButton.icon(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: isCaptured ? Colors.green : AppColors.navy, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), minimumSize: const Size(80, 32), elevation: 0),
          ),
        ],
      ),
    );
          ...
          : ListView(
              ...
              children: [
                ...
                if (currentStatus == 'ACCEPTED') ...[
                  AppPrimaryButton(label: 'START ARRIVING', onPressed: () { changeStatus('ARRIVING'); }),
                ],
                if (currentStatus == 'ARRIVING') ...[
                  AppPrimaryButton(label: 'MARK ARRIVED', onPressed: () { changeStatus('ARRIVED'); }),
                ],
                  ...
                  AppPrimaryButton(
                    ...
                    onPressed: () {
                      if (areAllPreTripPhotosCaptured) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please capture all 4-side photos and driver selfie first!'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
                      }
                    },
                  ),
                ],
                  ...
                  AppPrimaryButton(
                    ...
                    onPressed: () {
                      if (areAllPostTripPhotosCaptured) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please capture all 4 post-trip photos first!'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
                      }
                    },
                  ),
                ],
                ...
                if (showContact) ...[
                  AppOutlineButton(
                    label: customerPhone == null ? 'CONTACT CUSTOMER' : 'CALL CUSTOMER',
