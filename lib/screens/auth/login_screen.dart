...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> handleSendOtp() async {
    ...
    try {
      ...
      if (id.isEmpty) throw StateError('MSG91 did not return a request ID.');
      requestId = id;
      if (!mounted) return;
        ...
        builder: (context, setDialogState) {
          Future<void> verify() async {
            ...
            if (otp.length != 4 && otp.length != 6) {
              showMessage('Please enter the OTP sent to your WhatsApp.');
              return;
            }
            final id = requestId;
            ...
            try {
              await _otpService.verifyOtp(requestId: id, otp: otp, phoneNumber: phone);
              if (!mounted) return;
          ...
          Future<void> retryWhatsApp() async {
            final id = requestId;
            if (id == null || id.isEmpty) return;
            setDialogState(() => retrying = true);
            try {
              final response = await _otpService.retryViaWhatsApp(id);
              final newId = _extractRequestId(response);
              if (newId.isNotEmpty) requestId = newId;
              showMessage('A new OTP has been requested on WhatsApp.');
            } catch (e) {
              showMessage(e.toString().replaceFirst('StateError: ', ''));
            } finally {
          ...
          Future<void> retrySms() async {
            ...
            try {
              ...
              final newId = _extractRequestId(response);
              if (newId.isNotEmpty) requestId = newId;
              showMessage('OTP requested by SMS.');
            } catch (e) {
              showMessage(e.toString().replaceFirst('StateError: ', ''));
            } finally {
              setDialogState(() => retrying = false);
          ...
          return AlertDialog(
            ...
            title: const Text('Verify your number', style: TextStyle(fontWeight: FontWeight.w900, color: AppColors.navy)),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('We sent a verification code to your WhatsApp.', style: TextStyle(color: AppColors.muted, height: 1.35)),
                ...
                if (retrying)
                  ...
                  Row(children: [
                    TextButton(onPressed: retryWhatsApp, child: const Text('Retry WhatsApp')),
                    const Spacer(),
                    TextButton(onPressed: retrySms, child: const Text('Use SMS')),
                  ]),
              ],
            ),
            actions: [
              TextButton(onPressed: verifying ? null : () => Navigator.of(dialogContext).pop(), child: const Text('Cancel')),
              ElevatedButton(
              ...
              children: [
                Center(child: Column(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(color: AppColors.navy, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: AppColors.navy.withOpacity(0.3), blurRadius: 12, offset: const Offset(0, 6))]),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: Colors.grey.shade200), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 15, offset: const Offset(0, 5))]),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Container(
                      ...
                      child: Row(children: [
                        Expanded(child: GestureDetector(onTap: () => setState(() => isSignUp = false), child: Container(padding: const EdgeInsets.symmetric(vertical: 12), alignment: Alignment.center, decoration: BoxDecoration(color: !isSignUp ? AppColors.navy : Colors.transparent, borderRadius: BorderRadius.circular(11)), child: Text('Sign In', style: TextStyle(color: !isSignUp ? Colors.white : AppColors.muted, fontWeight: FontWeight.w900, fontSize: 13))))),
                        Expanded(child: GestureDetector(onTap: () => setState(() => isSignUp = true), child: Container(padding: const EdgeInsets.symmetric(vertical: 12), alignment: Alignment.center, decoration: BoxDecoration(color: isSignUp ? AppColors.navy : Colors.transparent, borderRadius: BorderRadius.circular(11)), child: Text('Register', style: TextStyle(color: isSignUp ? Colors.white : AppColors.muted, fontWeight: FontWeight.w900, fontSize: 13))))),
                      ]),
                    ),
                    ...
                    if (isSignUp) ...[
                      TextField(controller: _nameController, textCapitalization: TextCapitalization.words, decoration: InputDecoration(labelText: 'Full Name', prefixIcon: const Icon(Icons.person_outline_rounded, color: AppColors.navy), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.grey.shade300)))),
                      const SizedBox(height: 16),
                    ],
                    TextField(controller: _phoneController, keyboardType: TextInputType.phone, maxLength: 10, decoration: InputDecoration(labelText: 'Mobile Number', counterText: '', prefixText: '+91  ', prefixIcon: const Icon(Icons.phone_android_rounded, color: AppColors.navy), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.grey.shade300)))),
                    const SizedBox(height: 24),
                    SizedBox(width: double.infinity, child: ElevatedButton.icon(
                      style: ElevatedButton.styleFrom(backgroundColor: AppColors.navy, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0),
                      onPressed: isLoading ? null : handleSendOtp,
                      icon: const Icon(Icons.chat_rounded),
                      label: isLoading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Text('SEND OTP', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14, letterSpacing: 0.5)),
                    )),
                    const SizedBox(height: 12),
                    const Center(child: Text('WhatsApp OTP • SMS fallback available', style: TextStyle(color: AppColors.muted, fontSize: 11))),
                  ]),
                ),
              ],
            ),
