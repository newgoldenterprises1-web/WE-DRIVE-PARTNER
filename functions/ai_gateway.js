...
const ALLOWED_TOOLS = new Set([
  'get_booking', 'get_available_drivers', 'get_driver_status', 'get_customer',
  'create_job', 'assign_driver', 'send_notification', 'get_business_report', 'get_system_health',
]);
const READ_ONLY_TOOLS = new Set([
  'get_booking', 'get_available_drivers', 'get_driver_status', 'get_customer',
  'get_business_report', 'get_system_health',
]);
const ARGUMENT_KEYS = {
...
function validateArgs(tool, args) {
  ...
  const allowed = new Set(ARGUMENT_KEYS[tool] || []);
  ...
  switch (tool) {
    ...
    case 'get_available_drivers': return {
      serviceDate: text(args.service_date, 'service_date', 40), startTime: text(args.start_time, 'start_time', 40),
      location: text(args.location, 'location', 500), durationMinutes: args.duration_minutes == null ? null : finiteNumber(args.duration_minutes, 'duration_minutes'),
      latitude: optionalCoordinate(args.latitude, 'latitude', -90, 90), longitude: optionalCoordinate(args.longitude, 'longitude', -180, 180), requiredService: optionalText(args.required_service, 80),
    };
    case 'create_job': return {
      customerId: text(args.customer_id, 'customer_id', 160), serviceDate: text(args.service_date, 'service_date', 40), startTime: text(args.start_time, 'start_time', 40),
      location: text(args.location, 'location', 500), durationMinutes: args.duration_minutes == null ? null : finiteNumber(args.duration_minutes, 'duration_minutes'), notes: optionalText(args.notes, 1000),
    };
